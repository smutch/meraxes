...
#define ZETA_STOP (float) 60.1 // inclusive
#define ZETA_STEP (float) 10

/* Tvir min parameter space */
#define Tvir_START (float) 1000 //inclusive (in K)
  ...
  while (MFP < MFP_STOP){
	...
	while (Z < (ZEND+0.0001)){
	  ...
	  while ((Z < (ZEND+0.0001)) && (th_index < astro_num_threads)){
		...
		case 2:
		  if (USE_HALO_FIELD)
		    sprintf(cmnd, "delta_T -p %i %06.2f ../Boxes/xH_z%06.2f_nf*_eff%.1f_HIIfilter%i_Mmin%.1e_RHIImax%.0f_%i_%.0fMpc", proc_per_th, Z, Z, ZETA, HII_FILTER, M_MIN, MFP, HII_DIM, BOX_LEN);
		  else
		    sprintf(cmnd, "delta_T -p %i %06.2f ../Boxes/xH_nohalos_z%06.2f_nf*_eff%.1f_HIIfilter%i_Mmin%.1e_RHIImax%.0f_%i_%.0fMpc", proc_per_th, Z, Z, ZETA, HII_FILTER, M_MIN, MFP, HII_DIM, BOX_LEN);
		  break;
		default:
		  if (USE_HALO_FIELD)
		    sprintf(cmnd, "delta_T -p %i %06.2f ../Boxes/sphere_xH_z%06.2f_nf*_eff%.1f_HIIfilter%i_Mmin%.1e_RHIImax%.0f_%i_%.0fMpc", proc_per_th, Z, Z, ZETA, HII_FILTER, M_MIN, MFP, HII_DIM, BOX_LEN);
		  else
		    sprintf(cmnd, "delta_T -p %i %06.2f ../Boxes/sphere_xH_nohalos_z%06.2f_nf*_eff%.1f_HIIfilter%i_Mmin%.1e_RHIImax%.0f_%i_%.0fMpc", proc_per_th, Z, Z, ZETA, HII_FILTER, M_MIN, MFP, HII_DIM, BOX_LEN);
		}
	  ...
	  // now wait for all of the children to finish
	    ...
	    waitpid(pidarry[th_index],&status,0);
