  ...
  for (int i_burst = 1; i_burst < n_bursts; i_burst++) {
    ...
    if (m_stars > 1e-10) {
      ...
      if (m_stars_III > 1e-10)
        mlog("i_burst = %d, dull = %f, sn_III = %f", MLOG_MESG, i_burst, get_SN_energy_PopIII(i_burst, snapshot, 0) / ENOVA_CC, sn_energy_III / energy_unit * Number_SNII() * 1e10 / run_globals.params.Hubble_h);
      if (m_stars_II > 1e-10)
        mlog("sn_II = %f", MLOG_MESG, sn_energy_II);
