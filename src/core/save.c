...
void prepare_galaxy_for_output(galaxy_t gal, galaxy_output_t* galout, int i_snap)
{
  ...
  galout->MvirCrit_MC = (float)(gal.MvirCrit_MC); // Added by Manu for LW feedback
...
void calc_hdf5_props()
{
  ...
  if (!run_globals.params.FlagMCMC) {
    ...
    h5props->n_props = 50; // Manu: before was set to 49
    ...
    h5props->dst_offsets[i] = HOFFSET(galaxy_output_t, MvirCrit_MC); // Added by Manu for LW feedback
