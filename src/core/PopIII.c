...
void initialize_PopIII() // Initialize PopIII quantities that are easily computed just from the IMF.
{
  ...
  run_globals.params.physics.ReionNionPhotPerBaryIII =
    NionBaryIII; // Assign this value because this depends on the choice of the IMF
  ...
  mlog("Init IMF quantities: NPISN = %f, MPISN = %f, NSNII = %f, MSNII = %f, MBHs = %f",
       MLOG_MESG,
       run_globals.NumberPISN,
       run_globals.MassPISN,
       run_globals.NumberSNII,
       run_globals.MassSNII,
       run_globals.MassBHs);
}
...
double interp_mass(double lifetime) // Lifetime in yr units!!
{
  ...
  if (log10lifetime < run_globals.Time_Values[mass_bins - 1]) {
    ...
    massfinal_result =
      run_globals.Mass_Values[n_low] + ((log10lifetime - run_globals.Time_Values[n_low]) *
                                        (run_globals.Mass_Values[n_high] - run_globals.Mass_Values[n_low])) /
                                         (run_globals.Time_Values[n_high] - run_globals.Time_Values[n_low]);
  }
  return pow(10, massfinal_result); // Return result in SolarMass
}

double integrand_IMFnorm(double StarMass,
                         void* _) // You might want a case 2 for a different type of IMF (with Characteristic mass)
{
  int IMF_Type = run_globals.params.physics.PopIII_IMF;
...
double IMFnorm(double Mmin_IMF, double Mmax_IMF) // get normalization of Pop III IMF
{
  ...
  gsl_integration_qag(&F, Mmin_IMF, Mmax_IMF, 0, rel_tol, 1000, GSL_INTEG_GAUSS61, w, &norma, &normaerr);
  gsl_integration_workspace_free(w);

  return 1.0 / norma;
}

double getIMF(double StarMass, void* params)
{
  int IMF_Type = run_globals.params.physics.PopIII_IMF;
  ...
  if (IMF_Type < 3) { // Salpeter IMF
    ...
    double SigmaIMF = run_globals.params.physics.SigmaIMF;
    return 1.0 / StarMass *
           exp(log(Anorm) - 0.5 / (SigmaIMF * SigmaIMF) * log(StarMass / McharIMF) * log(StarMass / McharIMF));
  }
}

double getIMF_massweighted(double StarMass, void* params)
{
  return StarMass * getIMF(StarMass, params);
}
...
double get_StellarAge(double StarMass) // Star Mass in log10(Msol) to get tstar in log10(yr).
{
  ...
  switch (PopIIIAgePrescription) {
    case 1: // stellar lifetimes by Schaerer 2002 (popIII) model at met=0 with strong mass loss, mass range [80-1000]
            // Msun
      a0 = 8.795;
      a1 = -1.797;
      a2 = 0.332;
...
double Mass_BHs(void) // Add BHs for Pop III with M>40Msol. Atm they don't do anything, it's just because we don't want
                      // Stellar population surviving!
{
  double MmaxIMF = run_globals.params.physics.MmaxIMF;
  double MminIMF = run_globals.params.physics.MminIMF;
...
double CCSN_PopIII_Number(int i_burst, int curr_snap, int flagMW)
{
  ...
  double time_unit =
    run_globals.units.UnitTime_in_Megayears / run_globals.params.Hubble_h * 1e6; // You need result in yrs
  double m_min;
  double m_max;
  ...
  if (m_min > MmaxSnII) // There are no CCSN in this snapshot!
    ...
    gsl_integration_qag(&F, m_min, m_max, 0, rel_tol, 1000, GSL_INTEG_GAUSS61, w, &result, &err);
    gsl_integration_workspace_free(w);
...
double CCSN_PopIII_Fraction(
  int i_burst,
  int curr_snap,
  int flagMW) // from Mutch et al. 2016 flag == 0 Eq. 17, flag == 1 Eq. 22  Result in adimensional number
{
...
double CCSN_PopIII_Yield(
  int i_burst,
  int curr_snap,
  int yield_type) // 0 = Tot, 1 = Metals, 2 = Remnant. Based on Heger & Woosley 2010, No Mixing, S4 Model.
{
...
double PISN_PopIII_Yield(int yield_type) // Yield_type = 0 -> Recycling, 1 -> Metals
{
  ...
  if (yield_type == 0)
    ...
    mlog_error("Unrecognised value for yPISN PopIII yield_type parameter!");
    return PISN_MassFraction;
  }
}
