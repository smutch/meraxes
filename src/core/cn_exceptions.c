...
void cn_quote()
{
  ...
  strncpy(quote_list[30],
          "CHUCK_NORRIS_EXCEPTION: Users don’t mark Chuck Norris’s answers as accepted. The universe accepts them out "
          "of a sense of truth and justice.",
          max_quote_size);
  strncpy(quote_list[31], "CHUCK_NORRIS_EXCEPTION: Chuck Norris can divide by Zero.", max_quote_size);
  strncpy(quote_list[32],
          "CHUCK_NORRIS_EXCEPTION: Chuck Norris’s Stack Overflow reputation is only as modest as it is because of "
          "integer overflow (SQL Server does not have a datatype large enough).",
          max_quote_size);
  strncpy(quote_list[33],
          "CHUCK_NORRIS_EXCEPTION: Chuck Norris is the only top 100 Stack Overflow user who is human. The others are "
          "bots that he coded to pass the time between questions.",
          max_quote_size);
  strncpy(
    quote_list[34],
  ...
  strncpy(quote_list[35],
          "CHUCK_NORRIS_EXCEPTION: Chuck Norris does not use exceptions when programming. He has not been able to "
          "identify any of his code that is not exceptional.",
          max_quote_size);
  ...
  strncpy(quote_list[37],
          "CHUCK_NORRIS_EXCEPTION: Chuck Norris does not use revision control software. None of his code has ever "
          "needed revision.",
          max_quote_size);
  ...
  strncpy(
    ...
    max_quote_size);
  strncpy(
    quote_list[40], "CHUCK_NORRIS_EXCEPTION: When Chuck Norris points to null, null quakes in fear.", max_quote_size);
  ...
  strncpy(quote_list[42],
          "CHUCK_NORRIS_EXCEPTION: Chuck Norris took the red pill and the blue pill, and can phase-shift in and out of "
          "the Matrix at will.",
          max_quote_size);
  ...
  strncpy(quote_list[45],
          ...
          max_quote_size);
  strncpy(quote_list[46], "CHUCK_NORRIS_EXCEPTION: Chuck Norris does not sleep… He waits.", max_quote_size);
  strncpy(quote_list[47],
          "CHUCK_NORRIS_EXCEPTION: Chuck Norris doesn’t answer questions on Stack Overflow... He stares them down till "
  ...
  strncpy(quote_list[49],
          "CHUCK_NORRIS_EXCEPTION: Chuck Norris doesn’t need a debugger, he just stares down the bug until the code "
          "confesses.",
          max_quote_size);
  strncpy(
    quote_list[50],
  ...
  strncpy(quote_list[51],
          "CHUCK_NORRIS_EXCEPTION: Chuck Norris won the “Hello World in less than 20 bytes” contest by developing a "
          "single byte program.",
          max_quote_size);
  strncpy(quote_list[52],
          "CHUCK_NORRIS_EXCEPTION: Chuck Norris does not resolve software problems. The problems resolve themselves "
          "the moment he walks into the office.",
          max_quote_size);
  strncpy(quote_list[53],
          "CHUCK_NORRIS_EXCEPTION: Chuck Norris can answer a question well before it is asked and then get several "
          "up-votes whilst he has yet to finish typing the solution.",
          max_quote_size);
  strncpy(quote_list[54],
          "CHUCK_NORRIS_EXCEPTION: The Chuck Norris badge is awarded for posting a better answer than Chuck Norris. "
          "Only Chuck Norris can earn this badge.",
          max_quote_size);
  ...
  strncpy(quote_list[57],
          "CHUCK_NORRIS_EXCEPTION: When Chuck Norris presses Ctrl+Alt+Delete, a worldwide computer restart is "
          "initiated. The same goes for format.",
          max_quote_size);
  strncpy(quote_list[58], "CHUCK_NORRIS_EXCEPTION: Chuck Norris uses Visual Studio to burn CDs.", max_quote_size);
  ...
  strncpy(quote_list[61],
          "CHUCK_NORRIS_EXCEPTION: Chuck Norris dreams in ones and zeros. When two shows up, it is a nightmare. But "
          "again that’s only in theory. Two doesn’t exist for Chuck Norris.",
          max_quote_size);
  ...
  strncpy(quote_list[66],
          "CHUCK_NORRIS_EXCEPTION: Compatibility doesn’t exist in Chuck Norris’s dictionary. He can easily work in "
          "Microsoft Office in Linux on a Mac.",
          max_quote_size);
  strncpy(quote_list[67],
          "CHUCK_NORRIS_EXCEPTION: When Chuck Norris is programming, the Garbage Collector rests. The objects know "
          "when to destroy themselves.",
          max_quote_size);
  ...
  strncpy(quote_list[69],
          "CHUCK_NORRIS_EXCEPTION: When Chuck Norris is on a diet and doesn’t eat fast food, all hard disks change "
          "from FAT to NTFS.",
          max_quote_size);
  strncpy(quote_list[70],
          "CHUCK_NORRIS_EXCEPTION: Chuck Norris has written the best programming language. Its source has just one "
          "command… void Chuck NorrisSkeet();",
          max_quote_size);
  ...
  strncpy(quote_list[78],
          "CHUCK_NORRIS_EXCEPTION: There is simply no Halting Problem within a 10-meter radius of John Skeet, because "
          "computers ALWAYS halt in his presence.",
          max_quote_size);
  strncpy(quote_list[79],
          "CHUCK_NORRIS_EXCEPTION: Chuck Norris doesn’t look for reputation. Reputation looks for Chuck Norris.",
          max_quote_size);
  strncpy(quote_list[80], "CHUCK_NORRIS_EXCEPTION: Chuck Norris can do pair programming with himself.", max_quote_size);
  strncpy(quote_list[81],
          ...
          max_quote_size);
  strncpy(
    quote_list[82], "CHUCK_NORRIS_EXCEPTION: When Chuck Norris saves a file, the file thanks him.", max_quote_size);
  strncpy(quote_list[83],
          "CHUCK_NORRIS_EXCEPTION: Chuck Norris is immutable. If something’s going to change, it’s going to have to be "
          "the rest of the universe.",
          max_quote_size);
  strncpy(quote_list[84],
          "CHUCK_NORRIS_EXCEPTION: Chuck Norris’s addition operator doesn’t commute; it teleports to where he needs it "
          "to be.",
          max_quote_size);
  strncpy(quote_list[85],
          "CHUCK_NORRIS_EXCEPTION: Anonymous methods and anonymous types are really all called Chuck Norris. They just "
          "don’t like to boast.",
          max_quote_size);
  strncpy(quote_list[86],
          "CHUCK_NORRIS_EXCEPTION: Chuck Norris doesn’t have performance bottlenecks. He just makes the universe wait "
          "its turn.",
          max_quote_size);
  strncpy(quote_list[87],
          "CHUCK_NORRIS_EXCEPTION: Jeff Atwood bought a monster GPU just to calculate Chuck Norris’s rep on Stack "
          "Overflow. CPUs don’t cut it anymore.",
          max_quote_size);
  strncpy(
    quote_list[88],
  ...
  strncpy(quote_list[90],
          "CHUCK_NORRIS_EXCEPTION: When Chuck Norris presses F5, the Garbage collector collects itself.. there is no "
          "other garbage.",
          max_quote_size);
  strncpy(quote_list[91],
          "CHUCK_NORRIS_EXCEPTION: Chuck Norris once wrote an entire operating system in his sleep on a Treo with no "
          "battery, powered only by the force of his will.",
          max_quote_size);
  ...
  strncpy(quote_list[93],
          "CHUCK_NORRIS_EXCEPTION: If you have 10000 reputation points and Chuck Norris has 10000 reputation points, "
          "Chuck Norris has more reputation than you.",
          max_quote_size);
  strncpy(
    quote_list[94],
  ...
  strncpy(quote_list[96],
          "CHUCK_NORRIS_EXCEPTION: Chuck Norris codes only with final sealed methods. No one has ever needed to "
          "override any of Chuck Norris’s code.",
          max_quote_size);
  strncpy(quote_list[97], "CHUCK_NORRIS_EXCEPTION: Chuck Norris is IntelliSense.", max_quote_size);
  strncpy(quote_list[98], "CHUCK_NORRIS_EXCEPTION: Chuck Norris’s heart rate is 5 GHz.", max_quote_size);
  strncpy(quote_list[99],
          "CHUCK_NORRIS_EXCEPTION: Private methods in other libraries become public automatically once required in "
          "Chuck Norris’s code.",
          max_quote_size);
  strncpy(quote_list[100], "CHUCK_NORRIS_EXCEPTION: When Yoda needs advice, he calls Chuck Norris.", max_quote_size);
  ...
  strncpy(quote_list[102],
          "CHUCK_NORRIS_EXCEPTION: Norrised: The act of attempting to answer a Stack Overflow question only to find "
          "out that Chuck Norris has already answered it definitively and better than you would have ever done.",
          max_quote_size);
  strncpy(quote_list[103],
          "CHUCK_NORRIS_EXCEPTION: If Chuck Norris posts a duplicate question on StackOverflow, the original question "
