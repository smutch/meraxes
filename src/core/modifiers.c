...
void read_mass_ratio_modifiers(int snapshot)
{
  ...
  else
  {
    ...
    MPI_Bcast(run_globals.mass_ratio_modifier, sizeof(run_globals.mass_ratio_modifier), MPI_BYTE, 0, run_globals.mpi_comm);
  }
}
...
void read_baryon_frac_modifiers(int snapshot)
{
  if (strlen(run_globals.params.BaryonFracModifier) == 0)
  {
    ...
    MPI_Bcast(run_globals.baryon_frac_modifier, sizeof(run_globals.baryon_frac_modifier), MPI_BYTE, 0, run_globals.mpi_comm);
  }
}
