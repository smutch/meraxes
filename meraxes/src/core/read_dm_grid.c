    ...
    float         *grid_out)
{
    ...
    SID_log("Reading grid for snapshot %d ------ NEW VERSION WITH BUG FIX", SID_LOG_OPEN, snapshot);
    ...
    FILE *f1_pmg;
    char file1_pmg[128];
    sprintf(file1_pmg, "output-UVFB_ON_HEF-5_Snaps-1-99_GOOD_TRY_AGAIN/delta_grid_stats_snap%d.dat", snapshot);
...
//  fread(&ma_scheme, sizeof(int), 1, fin);
//
//  SID_log("Reading grid for snapshot %d ------ OLD VERSION WITH BUG", SID_LOG_OPEN, snapshot);
//  SID_log("n_cell = [%d, %d, %d]", SID_LOG_COMMENT, n_cell[0], n_cell[1], n_cell[2]);
//  SID_log("box_size = [%.2f, %.2f, %.2f]", SID_LOG_COMMENT, box_size[0], box_size[1], box_size[2]);
//  SID_log("ma_scheme = %d", SID_LOG_COMMENT, ma_scheme);
//
...
//            for (int k = 0; k < HII_dim; k++)
//            {
//                delta_LRF = *(grid + HII_R_FFT_INDEX(i, j, k));
//                
//                delta_LRF_sum += delta_LRF;
//                
//                if (delta_LRF<delta_LRF_min) delta_LRF_min = delta_LRF;
...
//    FILE *f1_pmg;
//    char file1_pmg[128];
//    sprintf(file1_pmg, "output-UVFB_OFF_HEF-5_Snaps-1-99_BAD_RERUN/delta_grid_stats_snap%d.dat", snapshot);
//    f1_pmg = fopen(file1_pmg, "wt");
//    fprintf(f1_pmg, "min\t%g\n", delta_LRF_min);
//    fprintf(f1_pmg, "max\t%g\n", delta_LRF_max);
//    fprintf(f1_pmg, "mean\t%g\n", delta_LRF_mean);
