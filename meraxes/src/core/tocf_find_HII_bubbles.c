...
double RtoM(double R, int filter){
...
float find_HII_bubbles(float redshift)
{
  ...
  float pixel_mass = RtoM(L_FACTOR*box_size/(float)HII_dim, filter);  // 1e10 Msol/h
  ...
  fftwf_plan plan = fftwf_mpi_plan_dft_r2c_3d(HII_dim, HII_dim, HII_dim, deltax, (fftwf_complex *)deltax, SID_COMM_WORLD, FFTW_ESTIMATE);
  fftwf_execute(plan);
  fftwf_destroy_plan(plan);

  float *stars = run_globals.tocf_grids.stars;
  fftwf_complex *stars_filtered = run_globals.tocf_grids.stars_filtered;
  plan = fftwf_mpi_plan_dft_r2c_3d(HII_dim, HII_dim, HII_dim, stars, (fftwf_complex *)stars, SID_COMM_WORLD, FFTW_ESTIMATE);
  fftwf_execute(plan);
  fftwf_destroy_plan(plan);
  
  float *sfr = run_globals.tocf_grids.sfr;
  fftwf_complex *sfr_filtered = run_globals.tocf_grids.sfr_filtered;
  plan = fftwf_mpi_plan_dft_r2c_3d(HII_dim, HII_dim, HII_dim, sfr, (fftwf_complex *)sfr, SID_COMM_WORLD, FFTW_ESTIMATE);
  fftwf_execute(plan);
  fftwf_destroy_plan(plan);
  ...
  {
    deltax[ii] /= (float)total_n_cells;
    stars[ii] /= (float)total_n_cells;
    sfr[ii] /= (float)total_n_cells;
  }
