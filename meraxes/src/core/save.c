  ...
  int              i_snap)
{
  ...
  galout->FescWeightedEBHM   = (float)(gal.FescWeightedEBHM);
...
void calc_hdf5_props(run_globals_t *run_globals)
{
  ...
  h5props->dst_offsets[i]     = HOFFSET(galaxy_output_t, BlackHoleGrossMass);
  h5props->dst_field_sizes[i] = sizeof(galout.BlackHoleGrossMass);
  ...
  h5props->field_units[i]     = "1e60photons";
  h5props->field_h_conv[i]    = "None";
  h5props->field_types[i++]   = H5T_NATIVE_FLOAT;
  ...
  h5props->dst_offsets[i]     = HOFFSET(galaxy_output_t, MergSnap);
  h5props->dst_field_sizes[i] = sizeof(galout.MergSnap);
  h5props->field_names[i]     = "MergSnap";
  h5props->field_units[i]     = "None";
  h5props->field_h_conv[i]    = "None";
  h5props->field_types[i++]   = H5T_NATIVE_INT;
