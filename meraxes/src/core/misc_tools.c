...
static void find_missing_gals(run_globals_t *run_globals, fof_group_t *fof_group, int NFof, int flag)
{
  ...
  SID_log("Running `find_missing_galaxies`...", SID_LOG_OPEN, SID.My_rank);
  ...
  } else if(flag==1)
  {
    ...
    SID_log("I find counter=%d using FOF traversal...", SID_LOG_COMMENT, SID.My_rank, counter);
    ...
    SID_log("I find %d gals traversing global list...", SID_LOG_COMMENT, SID.My_rank, master_counter);
  ...
  for(int ii=0; ii<counter; ii++)
    if(!gal_found[ii])
    {
      SID_log("ii = %d", SID_LOG_COMMENT, SID.My_rank, ii);
      missing_counter++;
    }
  ...
  SID_log("I find %d gals with ghost_flag=true", SID_LOG_COMMENT, SID.My_rank, counter);
  counter = 0;
  for(int i_fof=0; i_fof<NFof; i_fof++)
  {
    halo = fof_group[i_fof].FirstHalo;
  ...
  SID_log("I find %d gals with ghost_flag=true (FOF traversal)", SID_LOG_COMMENT, SID.My_rank, counter);
...
void check_counts(run_globals_t *run_globals, fof_group_t *fof_group, int NGal, int NFof)
{
  ...
  SID_log("Running counts check...", SID_LOG_OPEN|SID_LOG_TIMER, SID.My_rank);

  SID_log("NFof = %d", SID_LOG_COMMENT, SID.My_rank, NFof);
  SID_log("NGal = %d", SID_LOG_COMMENT, SID.My_rank, NGal);
  SID_log("NGhosts = %d", SID_LOG_COMMENT, SID.My_rank, run_globals->NGhosts);
  ...
  SID_log("Counting using gal->Next gives %d gals (-%d ghosts = %d gals)",
      SID_LOG_COMMENT, SID.My_rank, counter, run_globals->NGhosts,
  ...
  SID_log("Counting using FOF groups gives %d gals in %d halos", SID_LOG_COMMENT, SID.My_rank, counter, halo_counter);
  SID_log("%d halos are populated with at least one galaxy", SID_LOG_COMMENT, SID.My_rank, halo_pop_count);
